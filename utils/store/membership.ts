...
export function getNextTierBenefits(
  ...
  const nextTier = allTiers.find(
    tier => tier.minPoints > currentMembership.minPoints
  )
...
export function getProgressToNextTier(
  ...
  if (!currentMembership) {
    ...
    return Math.min(
      100,
      Math.max(0, (currentPoints / firstTier.minPoints) * 100)
    )
  }
  ...
  const pointsNeededForNextTier =
    nextTier.minPoints - currentMembership.minPoints
