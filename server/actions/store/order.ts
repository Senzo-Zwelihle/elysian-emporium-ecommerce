...
export async function createOrderAction(
  ...
  try {
    ...
    const order = await prisma.$transaction(async tx => {
      ...
      const pointsToAward = calculateOrderPoints(
        Number(serverCalculatedTotals.totalAmount)
      )
      await awardPoints(
        user,
        pointsToAward,
...
export async function awardReviewPointsAction(
  userId: string,
  productId: string
) {
  try {
